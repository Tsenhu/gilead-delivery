...
{
  #force to 5 digit siteid
  if(nchar(df[5,2])==3){df[5,2]=paste("00",df[5,2],sep="")}
  else if(nchar(df[5,2])==4){df[5,2]=paste("0",df[5,2],sep="")}
  else if(nchar(df[5,2])>5){flag=paste(flag,"Site Id out of length; ")}
  #get rid of ":"
  df[,1]=gsub("\\:.*","",df[,1])
...
for(i in 1:length(list_files))
{
  ...
  new_report_log[i,3]=unique(paste(temp_site_staff$`site Site Number`))
  new_report_log[i,4]=unique(paste(temp_site_staff$`site Investigator Last Name`, temp_site_staff$`site Investigator First Name`))
    ...
    if(sum(temp_site_staff[,c(1:11,13:16)]=="")>0)
  {
    ...
    if(sum(temp_site_drug[,c(17:24,26:29)]=="")>0)
    {
      ...
      if(sum(temp_site_staff[1,c(31:38)]=="No")==8)
      {
        ...
        }else
  {
    ...
    for(j in 1:nrow(temp_site_staff))
    {
      ...
      {
        temp_flag=append(temp_flag,"Email Missing; ")
...
#========Bracket_TXRS_Site User Import(1 tab)
  ...
  new_bracket_site_user=select(new_bracket_site_user,Date, `site Country`, `site Site Number`, `First Name`, `Last Name`, `User Type`, `E-mail`,Fax, Phone)
...
#========Brackete_IXRS_Site Import(2 tabs)
  ...
  new_bracket_site_additional=unique(select(temp_new_bracket_site_additional, Date, `site Country`, `site Site Number`, `New Role`, `First Name`, `Last Name`, `E-mail`))
  ...
  new_bracket_site_drug=select(temp_new_bracket_site_drug,`Add/Update Date`=Date, Country=`site Country`, SiteID=`site Site Number`, `Screening Status`, `Randomization Status`,
...
#  epro_contacts=sqldf(sql_epro)
  ...
  new_epro_contacts=select(mutate(filter(total_site_staff,grepl('Principal Investigator', `Specify Role`), `site Protocol No`==temp_protocol), Parent='CRO', `Domain Type`='Site', `Principal Investigator MIDDLE Name`='',
                                  ...
                                  , Parent,`Domain Type`, `Site Number`=`site Site Number`,
...
#=======================================================================================  
    ...
    {
      Bracket_Site_User_Import_hist=read.xlsx2(paste("Bracket_Site_User_Import ",temp_protocol,".xlsx",sep="") ,sheetName = "Site_User", check.names=FALSE)
      
      Bracket_Site_additional_hist=read.xlsx2(paste("Bracket_Site_Import ",temp_protocol,".xlsx",sep="") ,sheetName = "Additional Contacts", check.names=FALSE)
      
      Bracket_Site_drug_hist=read.xlsx2(paste("Bracket_Site_Import ",temp_protocol,".xlsx",sep="") ,sheetName = "Drug Delivery Contacts", check.names=FALSE)
      
      Edc_site_approval_hist=read.xlsx2(paste("EDC_Site and User Approval ",temp_protocol,".xlsx",sep="") ,sheetName = "Site Approval", check.names=FALSE)
      
      Edc_user_approval_hist=read.xlsx2(paste("EDC_Site and User Approval ",temp_protocol,".xlsx",sep="") ,sheetName = "User Approval", check.names=FALSE)
      
      Epro_contacts_hist=read.xlsx2(paste("ePRO ",temp_protocol,".xlsx",sep=""),sheetName = "ePRO", check.names=FALSE)
      
      Covance_hist=read.xlsx2(paste("Covance ", temp_protocol,".xlsx", sep=""),sheetName="Site Information", check.names=FALSE)
      
      Robarts_hist=read.xlsx2(paste("Robarts", temp_protocol,".xlsx", sep=""),sheetName = "Site Information", check.names=FALSE)
...
#=============================================================================================================    
  ...
  ####Bracket
  write.xlsx(bracket_site_user_import, paste("Bracket_Site_User_Import ",temp_protocol,".xlsx",sep="") ,sheetName = "Site_User",append=FALSE,row.names=FALSE)
  
  write.xlsx(bracket_site_additional, paste("Bracket_Site_Import ",temp_protocol,".xlsx",sep="") ,sheetName = "Additional Contacts",append=FALSE,row.names=FALSE)
  
  write.xlsx(bracket_site_drug, paste("Bracket_Site_Import ",temp_protocol,".xlsx",sep="") ,sheetName = "Drug Delivery Contacts",append=TRUE,row.names=FALSE)
  
  ####EDC
  write.xlsx(edc_site_approval, paste("EDC_Site and User Approval ",temp_protocol,".xlsx",sep="") ,sheetName = "Site Approval",append=FALSE,row.names=FALSE)
  
  write.xlsx(edc_user_approval, paste("EDC_Site and User Approval ",temp_protocol,".xlsx",sep="") ,sheetName = "User Approval",append=TRUE,row.names=FALSE)
  
  ###ePRO
  write.xlsx(epro_contacts, paste("ePRO ",temp_protocol,".xlsx",sep=""),sheetName = "ePRO",append = FALSE, row.names = FALSE)
  
  ###Covance
  write.xlsx(covance, paste("Covance ", temp_protocol,".xlsx", sep=""),sheetName="Site Information", append=FALSE, row.names = FALSE)
  
  ###Robart
  write.xlsx(robarts, paste("Robarts", temp_protocol,".xlsx", sep=""),sheetName = "Site Information", append = FALSE, row.names = FALSE)
}
